...
final class EPGParser: NSObject {
    ...
    private static func parseDate(_ string: String) -> Date? {
        ...
        guard let year  = Int(string.prefix(4)),
              let month = Int(string.dropFirst(4).prefix(2)),
              let day   = Int(string.dropFirst(6).prefix(2)),
              let hour  = Int(string.dropFirst(8).prefix(2)),
              let min   = Int(string.dropFirst(10).prefix(2)),
              let sec   = Int(string.dropFirst(12).prefix(2)) else { return nil }

        var comps = DateComponents()
        comps.year = year; comps.month = month; comps.day = day
        comps.hour = hour; comps.minute = min;  comps.second = sec
        comps.timeZone = parseTimezone(String(string.dropFirst(15).trimmingCharacters(in: .whitespaces)))
    ...
    private static func parseTimezone(_ offset: String) -> TimeZone {
        ...
        guard offset.count == 5,
              ...
              let hours = Int(offset.dropFirst(1).prefix(2)),
              let minutes = Int(offset.dropFirst(3).prefix(2)) else {
